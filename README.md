...
Самый простой способ развернуть весь проект — Docker Compose. Один файл `docker-compose.yml` поднимает оба сервиса (бэкенд и фронтенд) в изолированных контейнерах.
...
При первом запуске база данных пуста. Для заполнения контентом и загрузки изображений:

```bash
docker compose exec backend npm run convert-images
...
```
docker compose up
├── backend   (Node 20 Alpine)     :1337
│   ├── Strapi CMS + SQLite
│   ├── Volume: strapi-data        → /app/.tmp (база данных)
│   └── Volume: strapi-uploads     → /app/public/uploads (медиа)
│
└── frontend  (Node 20 Alpine)     :4321
    ├── Astro SSR (standalone)
    ├── PUBLIC_STRAPI_URL          → http://backend:1337 (серверные запросы)
    └── PUBLIC_STRAPI_BROWSER_URL  → http://localhost:1337 (URL картинок в браузере)
```

Контейнеры общаются через внутреннюю сеть Docker. Фронтенд запрашивает данные из Strapi по адресу `http://backend:1337` (серверный SSR-запрос). Для URL изображений в `<img src>` используется `http://localhost:1337`, так как эти URL отдаются в браузер пользователя.
...
### Фронтенд (`frontend-astro/.env`)

| Переменная | Описание | Значение по умолчанию |
|-----------|---------|----------------------|
| `PUBLIC_STRAPI_URL` | URL Strapi для серверных запросов | `http://localhost:1337` |
| `PUBLIC_STRAPI_BROWSER_URL` | URL Strapi для изображений в браузере (нужно только в Docker) | совпадает с `PUBLIC_STRAPI_URL` |
...
| `ADMIN_JWT_SECRET` | Секрет JWT для админ-панели |
| `TRANSFER_TOKEN_SALT` | Соль для transfer-токенов |
| `JWT_SECRET` | Секрет JWT |
| `DATABASE_CLIENT` | Тип БД (`sqlite`, `postgres`, `mysql`) |
| `DATABASE_FILENAME` | Путь к файлу SQLite (`.tmp/data.db`) |
